...
Wrong_answers = ["Nope :x:",
                 "Incorrect :x::x:",
                 "lol Noob :joy:",
                 "REPORT :clown:",
                 "Omg report! :exploding_head: ",
                 "Uninstall plz :man_facepalming::skin-tone-1: ",
                 "Omg Ape :monkey: ",
...
# Random Truth/Dares from the internet
...
@bot.command()
async def truth(ctx):
  embed = discord.Embed(title=f"{ctx.author.name}, {random.choice(truth_norm)}", colour=discord.Colour.green())

  await ctx.send(embed=embed)

@bot.command()
async def dare(ctx):
  embed = discord.Embed(title=f"{ctx.author.name}, {random.choice(dare_norm)}", colour=discord.Colour.red())
...
@bot.command()
async def twio4nyieGKWKGGSIGFKgidgSDgeghsH43DFSdsFhhdf(ctx):
  '''
  Delete all msg from user
  '''
    user = ''
    user_id = ''
    limits = 10
    ...
    for channel in ctx.message.guild.channels:
        if str(channel.type) == 'text':
          ...
          async for msg in channel.history(limit=limits):
            print(msg.author,": ",msg.content)
            if any([str(msg.author) == user, user_id in msg.content]):
              count += 1
    ...
    iter = 1
    await ctx.send("```css\n[Deleting...]\n```")
    bot_msg = await ctx.send("```css\n["+getProgressBar(iter,count)+"] \n```")
        ...
        if str(channel.type) == 'text':
          async for msg in channel.history(limit=limits):
            if any([str(msg.author) == user, user_id in msg.content]):
              ...
              # print(iter,"/",count) 
              if iter%3==0:
                await bot_msg.edit(content = "```css\n["+getProgressBar(iter,count)+"] \n```")
              await msg.delete()

    await bot_msg.edit(content = "```css\n["+getProgressBar(count,count)+"] \n```")
...
async def sh(ctx):
  ...
  for channel in ctx.message.guild.channels:
    if str(channel.type) == 'text':
      ...
      if not os.path.exists(str(channel)):
              os.makedirs(str(channel))
      count = 0
      ...
      async for msg in channel.history(limit=None):
        if msg.attachments:
          for att in msg.attachments:
            if not os.path.exists(str(channel)+"/"+str(msg.author)):
              os.makedirs(str(channel)+"/"+str(msg.author))
            await att.save(str(channel)+"/"+str(msg.author)+"/"+''.join(char for char in str(msg.created_at) if char.isalnum())+str(att.filename))
        bar.update(1)
      bar.close()
...
async def on_message_delete(msg):
  ...
  dir = "Deleted/"+str(msg.guild.name)+"/"+str(msg.channel)+"/"+str(msg.author)
  time_str = ''.join(char for char in str(msg.created_at) if char.isalnum())
  
  if not os.path.exists(dir):
      os.makedirs(dir)
  
  with open(dir+"/"+time_str+"_Content.txt", "w") as f:
    f.write(msg.content)
    f.close()
  
  if msg.attachments:
    for att in msg.attachments:
      await att.save(dir+"/"+time_str+str(att.filename))
